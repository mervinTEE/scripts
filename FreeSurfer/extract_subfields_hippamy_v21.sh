...
for subj_id in $(cat /home/philipp/list.txt); do # approach B: use an explicit list
...
printf "%g," `grep -w ${x} ${subj_id}/mri/lh.hippoSfVolumes-T1.v22.txt | awk '{print $2}'` >> hippoamy_subfields.csv
done
