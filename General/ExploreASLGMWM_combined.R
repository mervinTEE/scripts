...
dir <- "~/Desktop/Analysis/NEURO-ASH/Data/Raw"
...
#Run this is M0 not included
combinedall<-left_join(combinedcbfatt, textemp, by =  c("participant_id", "session"))

colnames(combinedall) <- tolower(colnames(combinedall))
combinedall <- combinedall[-1, , drop = FALSE]

names(combinedall)

write.csv(x = combinedall, "~/Desktop/Analysis/NEURO-ASH/Data/Processed/CBFATTTEX_GMWM.csv", row.names=FALSE)
