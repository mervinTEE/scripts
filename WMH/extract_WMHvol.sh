...
for SUBJ in "${SUBJECTS[@]}"; do
    for TP in "${TP_DIRS[@]}"; do
        ...
        # --- Distance-based ---
        row="$row,$(get_volumes ${SUBJ_TP_DIR}/DWPV_dist_output/PVWMH_10mm.nii.gz)"
        row="$row,$(get_volumes ${SUBJ_TP_DIR}/DWPV_dist_output/DWMH_10mm.nii.gz)"
        row="$row,$(get_volumes ${SUBJ_TP_DIR}/DWPV_dist_output/PVWMH_13mm.nii.gz)"
        row="$row,$(get_volumes ${SUBJ_TP_DIR}/DWPV_dist_output/DWMH_13mm.nii.gz)"
